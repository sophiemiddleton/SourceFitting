...
TString filepath = "/pnfs/mu2e/scratch/users/hjafree/Calibfiles.root";
using namespace std;
...
int MakeSpecTree(){
  for(int numVal = 685; numVal < 887; numVal++){
    ...
    //create output tree
    Float_t spec, ratio, ntrig, stim, time, tErg, Truespec;
    TTree *specTree = new TTree("specTree","specTree");
    specTree->Branch("spec", &spec, "spec/F"); //Energy Deposited 
		specTree->Branch("truespec", &Truespec, "Truespec/F"); //True energy deposited in crystal
    ...
    ergTree -> SetBranchAddress("simEdep", &simEdep);
    ...
    for(unsigned int iEvt=0; iEvt<nEvt; iEvt++)
    {
      ...
      for(int icry=0; icry<nCry; icry++)
      {
        if(cryId[icry] == numVal)
        {
          ...
          sameSimEdep.push_back(simEdep[icry]);
        }
      }
      ...
      float ergTarget = 0.0;
      float ergRest   = 0.0;
      float TrueergTarget = 0.0;
      float TrueergRest   = 0.0;
      int nTarget = 0;
      ...
      //int TruenumHit = sameSimEdep.size();
      if(difTime > 20)
      {
